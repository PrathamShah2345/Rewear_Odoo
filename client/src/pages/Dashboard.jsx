...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br py-20 from-green-50 via-emerald-100 to-green-200 px-6 md:px-12 text-gray-800">
      <h2 className="text-3xl font-extrabold mb-6 text-emerald-800">Welcome, {currentUser?.email}</h2>
      ...
      <div className="bg-white/70 backdrop-blur-md rounded-xl p-4 mb-10 shadow">
        ...
        <p><span className="font-semibold">Points:</span> {points}</p>
      </div>

      {/* Uploaded Items */}
      <div className="mb-12">
        <h3 className="text-2xl font-bold mb-4 text-emerald-700">My Uploaded Items</h3>
        {myItems.length > 0 ? (
          ...
          <p className="text-gray-600 text-center">You haven’t uploaded anything yet.</p>
        )}
      </div>

      {/* Incoming Swap Requests */}
      <div className="mb-12">
        <h3 className="text-2xl font-bold mb-4 text-emerald-700">Incoming Swap Requests</h3>
        {swapRequests.length > 0 ? (
          <div className="space-y-4">
            {swapRequests.map((req) => (
              <div key={req.id} className="bg-white/80 p-4 rounded-xl shadow border">
                <p><strong>From:</strong> {req.from}</p>
                <p><strong>For Item:</strong> {req.itemTitle}</p>
          ...
          <p className="text-gray-600 text-center">No swap requests received.</p>
        )}
      </div>

      {/* Outgoing Swap Requests */}
      <div>
        <h3 className="text-2xl font-bold mb-4 text-emerald-700">My Sent Swap Requests</h3>
        {sentRequests.length > 0 ? (
          ...
          <p className="text-gray-600 text-center">You haven’t sent any swap requests yet.</p>
        )}
      </div>
    </div>
  );
